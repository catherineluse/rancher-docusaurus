...
For more best practices for downstream clusters, refer to the [production checklist](https://rancher.com/docs/rancher/v2.6/en/cluster-provisioning/production) or our [best practices guide.](https://rancher.com/docs/rancher/v2.6/en/best-practices/)

# Architecture for an Authorized Cluster Endpoint 

If you are using an [authorized cluster endpoint,](https://rancher.com/docs/rancher/v2.6/en/overview/architecture/#4-authorized-cluster-endpoint) we recommend creating an FQDN pointing to a load balancer which balances traffic across your nodes with the `controlplane` role.

If you are using private CA signed certificates on the load balancer, you have to supply the CA certificate, which will be included in the generated kubeconfig file to validate the certificate chain. See the documentation on [kubeconfig files](https://rancher.com/docs/rancher/v2.6/en/cluster-admin/cluster-access/kubectl/) and [API keys](https://rancher.com/docs/rancher/v2.6/en/user-settings/api-keys/#creating-an-api-key) for more information.