...
\> Warning: private clusters require additional planning and configuration outside of Rancher. Refer to the [private cluster guide](https://rancher.com/docs/rancher/v2.6/en/cluster-provisioning/hosted-kubernetes-clusters/gke/private-clusters/).
...
\> Note: the default option is "Container-Optimized OS with Docker". The read-only filesystem on GCP's Container-Optimized OS is not compatible with the [legacy logging](https://rancher.com/docs/rancher/v2.0-v2.4/en/cluster-admin/tools/logging) implementation in Rancher. If you need to use the legacy logging feature, select "Ubuntu with Docker" or "Ubuntu with Containerd". The [current logging feature](https://rancher.com/docs/rancher/v2.6/en/logging) is compatible with the Container-Optimized OS image.
